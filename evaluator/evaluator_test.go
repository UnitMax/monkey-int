...
func TestErrorHandling(t *testing.T) {
	tests := []struct {
		...
		{
			`{"name": "Monkey"}[fn(x) { x }];`,
			"unusable as hash key: MONKEY_FUNC",
		},
	}
...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not a Function. Got=%T (%+v) instead", evaluated, evaluated)
	}
...
func TestClosures(t *testing.T) {
	input := "let newAdder = fn(x) { return fn(y) {x+y}; }; let addTwo = newAdder(2); addTwo(8);"
...
func TestStringLiteral(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not String. Got=%T (%+v) instead.", evaluated, evaluated)
	}
	if str.Value != "hello world" {
		t.Errorf("String has the wrong value. Expected=%q, got=%q instead.", "hello world", str.Value)
	}
}
...
func TestStringConcatenation(t *testing.T) {
	...
	str, ok := evaluated.(*object.String)
	if !ok {
		t.Fatalf("object is not a String. Got=%T (%+v) instead.", evaluated, evaluated)
	}
	if str.Value != "Hello World!" {
		t.Errorf("String has wrong value. Wanted=%q, got=%q instead.", "Hello World!", str.Value)
	}
}
...
func TestBuiltinFunctions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		case string:
			errorObj, ok := evaluated.(*object.Error)
			if !ok {
				t.Errorf("object is not an Error. Got=%T (%+v) instead.", evaluated, evaluated)
				continue
			}
...
func TestArrayLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not an Array. Got=%T (%+v) instead.", evaluated, evaluated)
	}
	if len(result.Elements) != 3 {
		t.Fatalf("Array has incorrect number of elements. Wanted=%d, got=%d instead.", 3, len(result.Elements))
	}
...
func TestHashLiterals(t *testing.T) {
	...
	if !ok {
		t.Fatalf("Eval didn't return a Hash. Got=%T (%+v) instead.", evaluated, evaluated)
	}
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong number of pairs. Expected=%d, got=%d instead", len(expected), len(result.Pairs))
	}

	for expectedKey, expectedVal := range expected {
		pair, ok := result.Pairs[expectedKey]
		if !ok {
			t.Errorf("No pair for given key in Pairs found.")
		}
